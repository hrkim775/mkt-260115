...
server.tool(
  "duplicate_frame_dark_mode",
  "Clone a top-level frame (default: name matches Background / 백그라운드), place the copy to the right, and remap fills/strokes to a dark UI palette (surfaces + light text).",
  {
    nodeId: z
      .string()
      .optional()
      .describe(
        "Exact node id of the frame to clone. If omitted, the first top-level frame whose name matches namePattern is used."
      ),
    namePattern: z
      .string()
      .optional()
      .describe(
        'Regex string for matching a frame name on the current page (default: "백그라운드|background"). Ignored if nodeId is set.'
      ),
    gap: z
      .number()
      .optional()
      .describe("Horizontal gap in px between original and clone (default: 48)."),
    nameSuffix: z
      .string()
      .optional()
      .describe('Suffix appended to the clone name (default: " (Dark)").'),
  },
  async ({ nodeId, namePattern, gap, nameSuffix }: any) => {
    try {
      const result = await sendCommandToFigma("duplicate_frame_dark_mode", {
        nodeId,
        namePattern,
        gap,
        nameSuffix,
      ...
      return {
        content: [
          {
            type: "text",
            text: `Dark mode variant: "${typed.cloneName}" (${typed.cloneId}). Source: "${typed.sourceName}". ${typed.message || ""}`,
          },
        ],
      };
    } catch (error) {
