...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "duplicate_frame_dark_mode":
      return await duplicateFrameDarkMode(params);
...
function _mapSurfaceRGB(r, g, b) {
  const L = _luminance(r, g, b);
  if (L > 0.92) return { r: 0.071, g: 0.071, b: 0.071 };
  if (L > 0.75) return { r: 0.11, g: 0.11, b: 0.11 };
  if (L > 0.52) return { r: 0.16, g: 0.16, b: 0.16 };
  if (L > 0.32) return { r: 0.24, g: 0.24, b: 0.24 };
  if (L < 0.12)
    return {
      r: Math.min(1, r + 0.12),
      g: Math.min(1, g + 0.12),
...
function _mapTextRGB(r, g, b) {
  const L = _luminance(r, g, b);
  if (L < 0.42) return { r: 0.92, g: 0.92, b: 0.92 };
  if (L > 0.72) return { r: 0.62, g: 0.62, b: 0.62 };
  return { r: 0.82, g: 0.82, b: 0.82 };
}

async function _loadAllFontsDeep(node) {
  if (node.type === "TEXT") {
    if (node.fontName === figma.mixed) {
  ...
  if ("children" in node) {
    for (const c of node.children) {
      await _loadAllFontsDeep(c);
    }
  }
}

function _applyDarkVisualsToNode(node) {
  if (node.type === "TEXT") {
    ...
    for (const p of fills) {
      ...
      if (p.type === "SOLID" && p.color) {
        const c = _mapTextRGB(p.color.r, p.color.g, p.color.b);
        newFills.push({
          type: "SOLID",
          color: c,
  ...
  if ("fills" in node && node.fills !== figma.mixed && Array.isArray(node.fills) && node.fills.length) {
    const newFills = node.fills.map(function (p) {
      if (p.visible === false) return p;
      if (p.type === "SOLID" && p.color) {
        const c = _mapSurfaceRGB(p.color.r, p.color.g, p.color.b);
        return {
          type: "SOLID",
          color: c,
          opacity: p.opacity !== undefined ? p.opacity : 1,
      ...
      if (p.type === "IMAGE" || p.type === "VIDEO") {
        return {
          ...
          opacity: p.opacity !== undefined ? p.opacity * 0.85 : 0.9,
        };
      }
      if (p.type && String(p.type).indexOf("GRADIENT") === 0) {
        return {
          type: "SOLID",
          color: { r: 0.14, g: 0.14, b: 0.16 },
          opacity: p.opacity !== undefined ? p.opacity : 1,
        };
      }
      return p;
  ...
  if (
    ...
    node.strokes = node.strokes.map(function (p) {
      if (p.visible === false) return p;
      if (p.type === "SOLID" && p.color) {
        const L = _luminance(p.color.r, p.color.g, p.color.b);
        if (L > 0.55) {
          return {
            type: "SOLID",
            color: { r: 0.24, g: 0.24, b: 0.26 },
            opacity: p.opacity !== undefined ? p.opacity : 1,
          };
        }
      }
...
async function _applyDarkModeToTree(node) {
  try {
    _applyDarkVisualsToNode(node);
  } catch (err) {
    console.warn("dark mode skip", node.id, err && err.message);
  }
  if ("children" in node) {
    for (const c of node.children) {
      await _applyDarkModeToTree(c);
    }
  }
}

async function duplicateFrameDarkMode(params) {
  var nodeId = params && params.nodeId;
  var namePattern = params && params.namePattern;
  var gap =
    params && params.gap !== undefined && params.gap !== null
      ? Number(params.gap)
      : 48;
  var suffix =
    (params && params.nameSuffix) || " (Dark)";

  await figma.currentPage.loadAsync();

  var source = null;
  if (nodeId) {
    source = await figma.getNodeByIdAsync(nodeId);
    if (!source) {
      throw new Error("Node not found: " + nodeId);
    }
  } else {
    var patStr = namePattern || "(백그라운드|background)";
    var re;
    try {
      re = new RegExp(patStr, "i");
    } catch (e) {
      re = /백그라운드|background/i;
    }
    var okTypes = ["FRAME", "COMPONENT", "INSTANCE", "SECTION"];
    var candidates = figma.currentPage.children.filter(function (c) {
      return okTypes.indexOf(c.type) >= 0 && re.test(c.name);
    });
    if (candidates.length === 0) {
      throw new Error(
        'No top-level frame matched pattern "' +
          patStr +
          '". Open the correct page or pass nodeId.'
      );
    }
    source = candidates[0];
  }
  ...
  await _loadAllFontsDeep(clone);
  await _applyDarkModeToTree(clone);
  ...
  figma.notify("Dark mode copy: " + clone.name);
  ...
  return {
    ...
    message:
      "Cloned to the right and applied dark surfaces + light text. Tweak colors in Figma if needed.",
  };
}
